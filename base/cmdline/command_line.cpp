...
#include "str/std_str_ext.h"

#ifndef DCHECK
#	define DCHECK(x) __noop
#endif


base::CommandLine* base::CommandLine::current_process_commandline_ = NULL;
...
#endif
	size_t switch_prefix_count = _countof(kSwitchPrefixes);
...
#if defined(OS_WIN)
					command_line.AppendSwitchNative(base::WideToASCII(switch_string), switch_value);
#elif defined(OS_POSIX)
...
namespace base
{
	...
	void CommandLine::set_slash_is_not_a_switch() {
		...
		DCHECK(wcscmp(kSwitchPrefixes[_countof(kSwitchPrefixes) - 1], L"/") == 0);
		switch_prefix_count = _countof(kSwitchPrefixes) - 1;
	}
#endif
