...
namespace win {
		...
		bool CheckAndDuplicateHandle(HANDLE source, HANDLE* target) {
			...
			if (!::DuplicateHandle(::GetCurrentProcess(), source,
				...
				DUPLICATE_SAME_ACCESS)) {
					DBG(ERROR) << "Failed to duplicate a handle.";
					return false;
			}
			*target = temp;
			return true;
	...
	ScopedProcessInformation::Receiver ScopedProcessInformation::Receive() {
		DBG_IF(ERROR, IsValid()) << "process_information_ must be NULL";
	...
	bool ScopedProcessInformation::DuplicateFrom(
		const ScopedProcessInformation& other) {
			DBG_IF(ERROR, !IsValid()) << "target ScopedProcessInformation must be NULL";
			DBG_IF(ERROR, other.IsValid()) << "source ScopedProcessInformation must be valid";
