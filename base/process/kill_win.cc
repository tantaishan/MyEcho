...
namespace base {
	...
	bool KillProcess(ProcessHandle process, int exit_code, bool wait) {
		...
		if (result && wait) {
			...
			if (WAIT_OBJECT_0 != WaitForSingleObject(process, 60 * 1000))
				DBG(ERROR) << "Error waiting for process exit";
		} else if (!result) {
			DBG(ERROR) << "Unable to terminate process";
		}
		return result;
	}
	...
	bool KillProcessById(ProcessId process_id, int exit_code, bool wait) {
		HANDLE process = OpenProcess(PROCESS_TERMINATE | SYNCHRONIZE,
			...
			process_id);
		if (!process) {
			DBG(ERROR) << "Unable to open process " << process_id;
			return false;
		}
	...
	TerminationStatus GetTerminationStatus(ProcessHandle handle, int* exit_code) {
		...
		if (!::GetExitCodeProcess(handle, &tmp_exit_code)) {
			DBG(ERROR) << "GetExitCodeProcess() failed";
			if (exit_code) {
		...
		if (tmp_exit_code == STILL_ACTIVE) {
			...
			if (wait_result == WAIT_FAILED) {
				DBG(ERROR) << "WaitForSingleObject() failed";
			} else {
				DBG_IF(ERROR, WAIT_OBJECT_0 != wait_result) << "error";

				// Strange, the process used 0x103 (STILL_ACTIVE) as exit code.
				DBG(WARNING) << "Strange, the process used 0x103 (STILL_ACTIVE) as exit code";
				//NOTREACHED();
			}
	...
	bool WaitForSingleProcess(ProcessHandle handle, uint32 wait) {
		int exit_code;
	...
	//	DBG_IF(ERROR, process != GetCurrentProcess()) << "error";
