...
void C_ImageSet::AddImage (C_Image* newimage)
{
	...
	int n=0;
	for (;n<nImages;n++) pTempImg[n]=img[n];	// (pointer copy)
	delete[] img;
	img=pTempImg;
	img[n]=newimage;
	nImages++;
...
HWND C_AnimationWindow::Create (HWND hwndParent, HMENU id, C_ImageSet * is,
								DWORD dwStyle, LPCTSTR szClassName)
{
	...
	if (pAnimation->nImages>0)
		m_hWnd=CreateWindow (szClassName,_T(""),dwStyle,
			0,0,0,0,
			hwndParent,(HMENU)id,
...
// ****************************************************************************
int C_Image::LoadBMP (LPCTSTR szFileName)
{
	int n;
...
// *                        (c) Sept2000, Juan Soulie <jsoulie@cplusplus.com> *
// ****************************************************************************
int C_Image::SaveBMP (LPCTSTR szFileName)
{
	int n;
...
int C_ImageSet::LoadGIF (istream& giffile)
{
	int n;
	...
	try
	{
		...
		do
		{
			...
			if (charGot == 0x21)		// *A* EXTENSION BLOCK 
			{
				...
				char * pCompressedImage = new char [(size_t)(ImgEnd-ImgStart+4)];
...
// ****************************************************************************
int C_ImageSet::LoadGIF (LPCTSTR szFileName)
{
	// OPEN FILE
