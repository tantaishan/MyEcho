...
public:
	...
	// File Formats:
	int LoadBMP (LPCTSTR szFile);
	int SaveBMP (LPCTSTR szFile);
};
...
public:
	...
	// File Formats:
	int LoadGIF (LPCTSTR szFile);
	int LoadGIF (istream& giffile);
	int SaveGIF (LPCTSTR szFile) {return 0;};	// NOT IMPLEMENTED
};

#define DEFAULT_CLASSNAME _T("AnimationWindow")
...
public:
	...
	HWND Create (HWND hwndParent, HMENU id, C_ImageSet * imageset,
		DWORD dwStyle=WS_CHILD, LPCTSTR szClassName=DEFAULT_CLASSNAME);
